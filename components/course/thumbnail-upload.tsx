...
export function ThumbnailUpload({
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/courses/upload-thumbnail`, {
        method: "POST",
        body: formData,
      })
      ...
      setTimeout(() => {
        ...
        onUploadComplete(data.thumbnailUrl)
        setIsUploading(false)
      }, 500)
    } catch (error) {
      setUploadStatus("error")
      setIsUploading(false)
      onUploadError("Failed to upload thumbnail. Please try again.")
      setPreviewUrl(null)
    }
  }
