...
export function AssessmentScreen({
  ...
  const handleSubmit = () => {
    ...
    assessment.questions.forEach((question) => {
      ...
      // Add logic for checkbox type if needed
      if (question.type === "checkbox") {
        // For checkbox questions, you might need to compare arrays
              ...
              <div className="space-y-2">
                {currentQuestion.options.map((option, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    <Checkbox
                      id={`checkbox-${index}`}
                      checked={((currentAnswerValue as string[]) || []).includes(option)}
                      onCheckedChange={(checked) => {
                        const currentAnswers = (currentAnswerValue as string[]) || []
                        if (checked) {
