...
export function AssessmentScreen({
  ...
  useEffect(() => {
    ...
    if ((isCompleted || isPending) && previousScore !== undefined) {
      setShowStartModal(false)
      fetchSavedAnswers()
    }
  }, [assessment.id, assessment.timeLimit, isCompleted, isPending, previousScore])
    ...
    setAnswers((prev) => {
      ...
      if (existingAnswerIndex >= 0) {
        ...
        const newAnswer: Answer = {
          id: "", // This will be generated by the backend when saved
          answer: answer,
  ...
  const handleSubmit = () => {
    ...
    if (requiresManualGrading) {
      console.log("Assessment requires manual grading")
      onPending()
    }
  ...
  if (showStartModal && !isSubmitted && !isCompleted && !isPending) {
    return (
