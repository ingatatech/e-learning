    ...
    const fetchMyCourses = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const completedCourses = data.enrollments.filter((c: EnrolledCourse) => c.course.status === "completed").length
          const inProgressCourses = data.enrollments.filter((c: EnrolledCourse) => c.course.status === "in_progress").length
            ...
            <TabsContent value={activeTab} className="mt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredEnrollments.map((enrollment) => (
                  <Card key={enrollment.id} className="pt-0 overflow-hidden hover:shadow-xl hover:-translate-y-2 transition-all duration-300 group border-2 hover:border-primary/20 bg-gradient-to-b from-card to-card/50">
                    <div className="relative">
                      <img
                        src={enrollment.thumbnail || "/placeholder0.svg"}
                      ...
                      <div className="absolute top-2 left-2">
                        <Badge className={`${getLevelColor(enrollment.course.level)} shadow-md flex items-center gap-1`}>
                    ...
                    <CardContent className="pt-0">
                      <div className="flex items-center gap-2 mb-4">
                        <img
                          src={enrollment.course.instructor?.profilePicUrl || "/placeholder.svg"}
                          alt={`${enrollment.course.instructor?.firstName} ${enrollment.course.instructor?.lastName}`}
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4 text-xs">
                        <div className="flex items-center gap-1 p-2 bg-muted/50 rounded-lg">
                          ...
                          <span className="font-medium">{enrollment.course.completedLessons}/{enrollment.course.totalLessons}</span>
                        </div>
                        <div className="flex items-center gap-1 p-2 bg-muted/50 rounded-lg">
                          <Clock className="w-3 h-3 text-blue-500" />
                          <span className="font-medium">{enrollment.course.duration != 0 ? enrollment.course.duration + ' hours' : "Not set"}</span>
                        </div>
                        <div className="flex items-center gap-1 p-2 bg-muted/50 rounded-lg col-span-2">
                          <Target className="w-3 h-3 text-orange-500" />
                          <span className="font-medium">Language: {enrollment.course.language}</span>
                        </div>
                      </div>
                ...
                <div className="text-center py-16">
                  ...
                  <h3 className="text-xl font-bold mb-3">
                    {activeTab === "all" 
                      ...
                      : `No ${activeTab.replace('_', ' ')} enrollments`
                    }
                  </h3>
